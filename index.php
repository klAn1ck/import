...
/*    if((int)$data_line[122] == 120 OR (int)$data_line[122] == 100 OR (int)$data_line[122] == 250){
    ...
    $technical_info = array(
        ...
        10 => array( //A-i-O
            38 => $scanner_type_data[(int)$data_line[122]],//'Scanner type',//123 0-none, 1- tablet, 2-Sheetfed
    ...
    $ids = array();
    if ($data_line[27]) $ids['ciss'] = (int)$data_line[27];
    if ($data_line[37]) $ids['print_ciss'] = (int)$data_line[37];
    if ($data_line[48]) $ids['print_rc'] = (int)$data_line[48];
    if ($data_line[66]) $ids['cartridge'] = (int)$data_line[66];
    if ($data_line[82]) $ids['ink_set'] = (int)$data_line[82];
    /*&$ids = array(
        'ciss'=> $data_line[27],
